...
#        self.sc3d.setGeometry(2,2,378,348)
        ...
        self.warnStyle = "Background-color:rgb(255, 255, 112)"
        self.activeStyle = "Background-color:rgb(76, 250, 0)"
        self.normStyle = "Background-color:rgb(255, 255, 255)"
    ...
    def app_setup(self):
        ...
        self.ui.cbx_TimeMilliSecs.setCurrentIndex(1)
        ...
        self.ui.pbEndCapture.setEnabled(False)
        self.ui.pbStartCapture.setEnabled(True)
        self.ui.pbSavePtCloud.setEnabled(False)   
    ...
    def create_zed_camera_object(self):
        ...
        if err != sl.ERROR_CODE.SUCCESS :
            print(err)
            self.ui.lbl_ZEDstatus.setText("Offline")
            self.ui.lbl_ZEDstatus.setStyleSheet(self.warnStyle)
            self.ui.lbl_ZEDstatus.repaint() 
            self.zed.close()
            self.zed.close()
            exit(1)

        self.ui.lbl_ZEDstatus.setText("Ready")
        self.ui.lbl_ZEDstatus.setStyleSheet(self.activeStyle) 
         
        
        self.zedModel = "zedMini"
    ...
    def get_sensor_status(self):
        ...
        if self.hasHZlidar:
            self.ui.lbl_HZactive.setText("Active")
            self.ui.lbl_HZactive.setStyleSheet(self.activeStyle)
            self.ui.lbl_HZactive.repaint()        
        else:
            self.ui.lbl_HZactive.setText("Offline")
            self.ui.lbl_HZactive.setStyleSheet(self.warnStyle) 

        if self.hasVTlidar:
            self.ui.lbl_VTactive.setText("Active")
            self.ui.lbl_VTactive.setStyleSheet(self.activeStyle)
            self.ui.lbl_VTactive.repaint()        
        else:
            self.ui.lbl_VTactive.setText("Offline")
            self.ui.lbl_VTactive.setStyleSheet(self.warnStyle)     
    ...
    def  end_lidar_capture(self):
        ...
        self.export_collection_data_to_csv()
    ...
    def  start_lidar_capture(self):
        ...
        #// disable start button until end is clicked
        self.ui.pbStartCapture.setEnabled(False)
        self.ui.pbEndCapture.setEnabled(True)
    ...
    def get_hz_lidar_data(self):
        serHz = serial.Serial(port='/dev/ttyUSB0',
            baudrate=230400,
            timeout=5.0,
            bytesize=8,
            parity='N',
        ...
        self.fwdXY = str(round(self.xcrdsHz[self.idxH0], 3)) + "," + str(round(self.ycrdsHz[self.idxH0], 3))
        self.bkwdXY = str(round(self.xcrdsHz[self.idxH180], 3)) + "," + str(round(self.ycrdsHz[self.idxH180], 3))
        
        #// show distances inlist
        
        self.ui.lstDistances.clear()
        if int(vectorsHz[self.idxH0][1]) in range(140,160):
            self.ui.lbl_FwdDist.setStyleSheet(self.activeStyle)
            self.ui.lbl_FwdDist.setText(str(round(vectorsHz[self.idxH0][1], 3)) + " cm")
        elif int(vectorsHz[self.idxH0][1]) < 100:
            self.ui.lbl_FwdDist.setStyleSheet(self.warnStyle)
            self.ui.lbl_FwdDist.setText("TOO CLOSE")
        else:
            self.ui.lbl_FwdDist.setStyleSheet(self.normStyle)
            self.ui.lbl_FwdDist.setText(str(round(vectorsHz[self.idxH0][1], 3)) + " cm")
        
        self.ui.lstDistances.addItem(" Fwd: " + str(round(vectorsHz[self.idxH0][1],3)) + " cm" )
        self.ui.lstDistances.addItem(" Bck: " + str(round(vectorsHz[self.idxH180][1],3)) + " cm" )
        self.ui.lstDistances.addItem(" Lft: " + str(round(vectorsHz[self.idxH270][1],3)) + " cm" )
        self.ui.lstDistances.addItem("Rght: " + str(round(vectorsHz[self.idxH90][1],3)) + " cm" )
    ...
    def get_zed_pose_3(self):
        ...
        if tracking_state == sl.POSITIONAL_TRACKING_STATE.OK:
            ...
            self.imuQuat = sensors_data.get_imu_data().get_pose().get_orientation().get()
            ...
            #// calc imu dir
            iq0 = round(self.imuQuat[3], 3)
            iq1 = round(self.imuQuat[0], 3)
            iq2 = round(self.imuQuat[1], 3)
            iq3 = round(self.imuQuat[2], 3)  
            ...
            #// get side_by_side images
            image_sl_left = sl.Mat()
            self.zc.retrieve_image(image_sl_left, sl.VIEW.LEFT)
            image_cv_left = image_sl_left.get_data()
        
            image_sl_right = sl.Mat()
            self.zc.retrieve_image(image_sl_right, sl.VIEW.RIGHT)
            ...
            rghtimg = self.collFldr + "/r_" + str(self.ldrCapCnt) + ".png"
            lftimg = self.collFldr + "/l_" + str(self.ldrCapCnt) + ".png"
            
            cv2.imwrite(rghtimg, image_cv_right)
            cv2.imwrite(lftimg, image_cv_left)
            lpx = QPixmap(lftimg)
            img2 = lpx.scaled(370,220, QtCore.Qt.KeepAspectRatio)
            self.ui.lbl_ZEDimage.setPixmap(QPixmap(img2))
    ...
    #=========================
    def export_collection_data_to_csv(self):
        
        csvFile = self.collPath + "/" + self.collIdc + "/" + self.collIdc + ".txt"        
        
        cf = open(csvFile, 'w')
        delim = ";"
        
        #// export collection records for all tables
        con = sqlite3.connect(self.dbPath)
        cur = con.cursor() 
        
        #// get collection details record
        cf.write("=collection" + '\n')
        collSql = "select * from collection"\
            + " where collarea_idc = '" + self.curCollAreaIdc + "'"\
            + " and collection_id = " + str(self.collId)
            
        print(collSql)
        
        cur.execute(collSql)
        res = cur.fetchone()
        colltmp = list(map(str, res))
        collres = delim.join(colltmp)
        cf.write(collres + '\n')

#        #// get crack records for collection
#        cf.write("=crack" + '\n')
#        crkSql = "select * from crack"\
#            + " where collection_idc = '" + self.collIdc + "'"
#        
#        cur.execute(crkSql)
#        res2 = cur.fetchall()
#        if len(res2) == 0:
#            print("NO CRACKS")
#        for r2 in res2:
#            crktmp = list(map(str, r2))
#            crkres = delim.join(crktmp)
#            cf.write(crkres + '\n')
    
        #// get lidar records for collection
        cf.write("=lidar_data" + '\n')
        ldrSql = "select * from lidar_data"\
            + " where collarea_idc = '" + self.curCollAreaIdc + "'"\
            + " and coll_id = " + str(self.collId)
        
        cur.execute(ldrSql)
        res3 = cur.fetchall()
        for r3 in res3:
            ldrtmp = list(map(str, r3))
            ldrres = delim.join(ldrtmp)
            cf.write(ldrres + '\n')    
